...
**Current version:** `18.7.6` — **Patch:** Plan B on the Dolibarr `/holidays` saga — **cut the leaves sync from the payroll flow entirely**. After Walid confirmed the Leave Request Management module is enabled in the Dolibarr admin UI AND `llxvv_const` has `MAIN_MODULE_HOLIDAY=1, entity=1` (the table prefix on this install is `llxvv_`, not `llx_`, which is why the first SELECT came back empty), the `/api/index.php/holidays` endpoint still returns `HTTP/2 200`, `content-type: text/html`, body `"API not found (failed to include API file)"` with `x-proxy-cache: MISS` (the 18.7.4 cache-buster is working — this is genuinely PHP reaching the line-402 Dolibarr fallback, not a stale edge response). Four consecutive diagnosis attempts (18.7.1 → 18.7.5) couldn't find the actual cause and Walid said "lets do plan B — we need to move forward". So: payroll's `runSync()` in `payroll-periods-client.tsx` is now employees-only — the whole `/api/hr/leave-requests/sync` call, the soft-fail try/catch, the `leaveSyncWarning` state, the amber warning panel, the "Leaves" row in the status strip, the `LeaveSyncLog` type, `lastLeaveSync` state, and the parallel fetch in `loadLastSync()` are all removed. Hero description updated from "Sync fresh employee & leave data" to "Sync fresh employee data… leaves are read from the attendance sheet". Net effect: payroll behaves like pre-18.6.0 — calculates from `Employee` + `AttendanceRecord` sheet codes only — with one exception, native OTS `LeaveRequest` rows (if HR ever creates any through `/hr/leaves`) are still honored by the calculator because the dedup logic from 18.6.0 keys on `status=APPROVED` regardless of `source`. The underlying `/api/hr/leave-requests/sync` endpoint + the `DolibarrLeavesSyncScheduler` cron + the `/hr/leaves` page's own sync button are untouched — they still exist for whenever the Dolibarr-side issue is eventually resolved, they just don't block payroll anymore. No schema changes. The broken Dolibarr `/holidays` endpoint is now completely non-blocking.
