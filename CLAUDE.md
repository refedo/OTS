...
**Current version:** `18.16.0` — **Minor:** HR Module Enhancements & HR Setup Consolidation. HR Setup page fully redesigned with gradient sky/blue hero banner, 4 KPI tiles (departments, sections, position titles, leave types), and 3 new integration tabs — Attendance Sync (Google Sheet sync with probe + history), Dolibarr Sync (employee sync with reconciliation gate), and Identity Reconciliation (OTS↔Dolibarr user linking). Assignment log gains inline edit (PATCH `/api/hr/asset-assignments/[id]`) and clickable asset/employee name links. Backlog items gain Delete action (soft-delete via `DELETE /api/backlog/[id]`). Traffic violations form auto-populates employee's current car on employee selection. GitHub issues auto-close when backlog status changes to COMPLETED/DROPPED.
