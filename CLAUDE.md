...
**Current version:** `19.3.0` — **Minor:** Payroll period detail page enhancements. Excel (xlsx) export for payroll lines via `GET /api/hr/payroll-periods/[id]/export-xlsx`. Employee name is now a clickable link to the employee detail page. Sortable table headers on all columns. Overtime hours column added before overtime pay. Two new leave columns: "Leave w/ Permission (days / deduction)" and "Leave w/o Permission (days / deduction)" — deduction multipliers configurable in HR Setup → Payroll Settings (default: with-permission = 1×, without-permission = 2×). Loan deduction, custody deduction, and violation deduction columns now visible in the main payroll table. Traffic violations with `deductFromPayroll=true` and `status=PAID_BY_COMPANY` within the pay period date range are summed as `violationDeduction` on the PayrollLine. New `absenceWithPermissionDeduction` and `violationDeduction` fields added to `PayrollLine` (idempotent migration `add_payroll_leave_violation_deductions.sql`). `payroll.absenceWithPermissionMultiplier` and `payroll.absenceWithoutPermissionMultiplier` system config keys added to `PayrollSettings`. Hero banner, KPI strip, and totals footer row added to the payroll detail page. Full-width layout replaces the old centered layout. Payslip download `<a>` tag now sets an explicit filename via `download` attribute to prevent browsers saving PDFs as .HTM.
