...
**Current version:** `18.6.1` — **Patch:** HR UI polish + payroll sync button + Dolibarr holidays fallback. `/hr/leaves` and `/hr/payroll` are rebuilt on a shared design language (slate-50 → white gradient background, rounded-2xl gradient hero headers, 4 KPI tiles per page, slate-200 cards, pill-based sync counters, proper empty states and hover transitions). `/hr/payroll` gains its own "Sync from Dolibarr" button (gated by `hr.employee.sync` + `hr.leaves.sync`) that runs employee sync then leaves sync sequentially so both feeds are fresh before payroll calculation, with a dual-row status strip showing last employee + last leaves sync. `DolibarrClient.request()` no longer crashes on non-JSON responses — it now captures the raw text and produces a previewable error; `getHolidays()` / `getAllHolidays()` / `getHolidayTypes()` wrap missing-module cases in a typed `DolibarrHolidaysNotAvailableError`, and `POST /api/hr/leave-requests/sync` returns HTTP 503 with a user-friendly message ("Enable the Leaves module in Dolibarr...") instead of surfacing the raw `Unexpected token 'A', 'API not fo'...` parse error that appeared when the Dolibarr holidays REST endpoint was disabled.
