...
**Current version:** `18.2.0` — **Minor:** HR Phase 2.5 — dashboard, timesheet navigation, attendance mapping. New `/hr/dashboard` page with KPIs (headcount, regular/overtime/total hours, present/absent/vacation/sick days), group breakdown by occupation/section/department, stacked bar + daily trend + absence-mix charts, and date-range/filter controls — gated by `hr.employee.view`. `/hr/attendance/timesheet` is now a landing redirect to the first active employee; the timesheet header embeds an `EmployeePicker` searchable combobox so HR can hop between employees without backing out. New `/hr/attendance/mapping` UI + API routes resolve unknown worker columns from the Google Sheet: orphan columns become `AttendanceMappingCandidate` rows (UNMAPPED / RESOLVED / IGNORED) that HR can link to an employee or ignore as false positives; the sync service honors IGNORED and short-circuits on RESOLVED. `Employee` gained `occupation` / `section` / `departmentId` (FK to the existing `Department` model, not parallel free-text), managed directly in the employee form with a section dropdown (Preparation / Fabrication / Other). Idempotent manual migration `add_hr_phase_2_5.sql` with stored-procedure `information_schema.COLUMNS` guards.
