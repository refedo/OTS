...
**Current version:** `19.7.0` — **Minor:** Dolibarr employee-only sync filter + employee form extra fields + PTS attendance daily cron. Dolibarr sync now fetches only `employee=1` users (excludes admin/API accounts). Employee form gains a new "Documents & Contract" tab with 14 Dolibarr extrafields (boarderNumber, maritalStatus, occupationAr, gosiSubscriptionNo, contractEndDate, contractDuration, passportNumber, iqamaUrl, passportUrl, sponsorNumber, contractType, workingLocation, transferType, employeeNo). All 14 new fields wired through API PUT schema and TRACKED_SYNC_FIELDS. Daily PTS overtime sync cron job added via `AttendanceSyncScheduler` — runs at 06:00 Asia/Riyadh, gated by `ENABLE_ATTENDANCE_SYNC_SCHEDULER` env var, registered in cron registry and instrumentation.ts.

**Previous version:** `19.6.1` — **Patch:** DETAILING WorkUnit type added + build fix. `WorkUnitType` enum gains four new values — `DETAILING` (shop drawings, depends on DESIGN approval), `COATING` (surface treatment, depends on PRODUCTION), `DISPATCH` (dispatch & delivery, depends on COATING), `ERECTION` (site erection, depends on DISPATCH) — completing the full 8-stage steel fabrication sequence: DOCUMENTATION → DESIGN → DETAILING → PROCUREMENT → PRODUCTION → COATING → DISPATCH → ERECTION. MySQL migration `add_work_unit_types.sql` modifies the enum columns in `work_units` and `dependency_blueprint_steps`. Dependency blueprint seed updated with the correct 8-step FS dependency chain; the seed now auto-detects and updates stale existing blueprints that are missing DETAILING steps. EWS `WORK_UNIT_TYPE_TO_TRACKER_ACTIVITIES` mapping corrected: DESIGN now maps only to the `design` tracker activity (not `detailing`), each new type maps to its own tracker column. Build error in changelog fixed (unescaped apostrophe in JSX string).
