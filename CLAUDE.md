...
**Current version:** `19.16.1` — **Patch:** Three API resilience fixes for missing DB columns/tables. (1) `GET /api/settings` — if the `ceoSignatureUrl` column is missing (migration not yet run), falls back to an explicit select that excludes it and appends `ceoSignatureUrl: null` to the response so the settings page loads normally. (2) `GET /api/hr/letters/templates` — returns `[]` instead of 500 when the `HrLetterTemplate` table does not exist yet; the letters create dialog remains functional with no pre-filled templates. (3) `GET /api/hr/circulations` — returns `[]` when the `HrCirculation` table does not exist; `POST /api/hr/circulations` returns 503 with "please restart the server to apply migrations" rather than a generic 500. `POST /api/hr/letters` 500 was fixed in the same commit via a 3-tier fallback.

**Previous version:** `19.16.0` — **Patch:** Three API resilience fixes (see 19.16.1 above for details — 19.16.0 was a mis-tagged intermediate commit).

**Previous version:** `19.15.1` — **Patch:** Permission matrix search + RBAC/PBAC activation tooling. Search input added to the permission matrix page (filters by name, description, or permission ID in real-time; categories with no matches are hidden; match count shown inline; clear button resets filter). Standalone `scripts/run-permission-sync.js` added for server-side RBAC role sync and PBAC activation of linked employees.
