...
**Current version:** `18.6.3` — **Patch:** Revert the Dolibarr direct-MySQL holidays fallback per Walid's instruction ("the API is working perfectly for products, third parties, POs and financial sync, so there's no need for direct DB access — look up how the existing integrations work and do the same"). The OTS holidays REST call already follows the exact same pattern as `/products`, `/thirdparties`, `/invoices`, `/salaries`, `/projects`, `/supplierorders` and `/users` — same `DolibarrPaginationParams`, same `t.rowid` default sortfield, same `request<T>()` plumbing — so when `getHolidays()` fails the root cause is on the Dolibarr server, not in OTS. This patch deletes `src/lib/dolibarr/dolibarr-db.ts`, removes the `DOLIBARR_DB_HOST/PORT/USER/PASSWORD/DATABASE` env vars from `src/lib/env.ts`, strips the DB-fallback branch out of `runDolibarrLeaveSync()` so it calls `client.getAllHolidays()` exactly the way the products / thirdparties syncs call their endpoints, and rewrites `DolibarrHolidaysNotAvailableError`'s message to give administrators the three concrete server-side fixes (grant the API-key user the `holiday/read` permission, delete `htdocs/api/temp/routes.php` to clear Dolibarr's stale API-route cache, and verify `htdocs/holiday/class/api_holidays.class.php` exists + is readable by the PHP process). The `/api/hr/leave-requests/sync` 503 response now surfaces this message verbatim instead of telling admins to wire up MySQL credentials. Carries forward the 18.6.2 employee SN natural-numeric sort fix (1, 2, 10, 100 instead of 1, 10, 100, 2).
