...
**Current version:** `18.3.0` — **Minor:** HR Setup page + dashboard/timesheet polish. New `/hr/setup` page (tabbed: Departments + Sections) lets HR manage the dropdown lists used across the module — rename, reorder, archive. Gated behind `departments.create` / `departments.edit` / new `hr.section.manage` permission. New `HrSection` table + `Department.archivedAt` (idempotent manual migration `add_hr_setup.sql`). Renaming a section cascades to all employees holding the old value so existing records stay consistent; archived departments and sections disappear from pickers but historical references are preserved. The employee form now fetches sections live from `/api/hr/sections` instead of the hardcoded list, and still preserves a current employee's archived section so edits never silently drop it. `/hr/dashboard` and `/hr/attendance/timesheet/*` got a visual refresh: gradient hero header, refined KPI cards with icon chips + colored accent bars + gradient backgrounds, larger day-grid cells with hover elevation, sticky filter bar, tabular numerics, max-width containers. One-shot `scripts/update-hr-setup-permissions.ts` seeds `hr.section.manage` onto existing CEO + HR roles.
