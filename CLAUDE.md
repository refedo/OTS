...
**Current version:** `18.4.0` — **Minor:** Division + Occupation as managed dropdowns, sortable employee list, dynamic filter counter. New `HrDivision` + `HrOccupation` tables follow the same shape as `HrSection` (name, displayOrder, archivedAt). `Employee.division` column added. The employee form's Occupation input is now a live Select fed from `/api/hr/occupations`, and a new Division Select sits next to Section. `/hr/setup` gains two new tabs (Divisions + Occupations), both reusing a new generic `ManagedListTab` component — all three managed lists are gated behind the existing `hr.section.manage` permission. Renaming a division or occupation cascades through every employee holding the old value, matching the section pattern. Migration `add_hr_divisions_and_occupations.sql` uses the stored-procedure guard for the new column + `CREATE TABLE IF NOT EXISTS` for the new tables + seeds `HrOccupation` from existing `Employee.occupation` values so HR starts with a populated dropdown. `division` is in `TRACKED_SYNC_FIELDS` so manual edits survive Dolibarr sync. Employees list now has click-to-sort headers (ID / Name / Trade / Department / Status / Joined / Basic Salary / Sync) and a live "Showing X of Y employees" counter that reacts to the status, trade, and search filters.
