...
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_qms_processes;
DELIMITER $$
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM ImsQmsProcess WHERE processNumber = 'PROC-006') THEN
    ...
    VALUES (UUID(), 'PROC-006', 'Coating & Painting', 'QC Inspector', 'CORE',
      ...
      'Coated members with DFT per project spec (typically epoxy primer + alkyd topcoat)',
  ...
  IF NOT EXISTS (SELECT 1 FROM ImsQmsProcess WHERE processNumber = 'PROC-012') THEN
    ...
    VALUES (UUID(), 'PROC-012', 'In-House Roll Forming — Cladding Panels', 'Production Manager', 'IN_HOUSE',
      ...
      'Profile dimensional accuracy +/-1.5 mm, scratch/dent rate < 0.3%, daily output per line >= 2000 lm',
      'ISO 9001 §8.5.1', 'ACTIVE',
...
END$$
DELIMITER ;
CALL seed_qms_processes();
DROP PROCEDURE IF EXISTS seed_qms_processes;

-- ──────────────────────────────────────────────────────────────────────────────
-- 2. Approved Supplier List (FRM-003)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_approved_suppliers;
DELIMITER $$
...
END$$
DELIMITER ;
CALL seed_approved_suppliers();
DROP PROCEDURE IF EXISTS seed_approved_suppliers;

-- ──────────────────────────────────────────────────────────────────────────────
-- 3. Training Needs Analysis (FRM-005)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_training_needs;
DELIMITER $$
CREATE PROCEDURE seed_training_needs()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Ahmed Al-Rashidi' AND requiredTraining LIKE 'ASNT RT Level II%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Ahmed Al-Rashidi', 'QC', 'Senior QC Inspector',
      'No current RT Level II certification; performing UT only',
      'ASNT RT Level II Certification Course',
      'HIGH', '2025-09-30', 'OPEN',
      'Client projects now require RT Level II on-site sign-off.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Mohammed Al-Qahtani' AND requiredTraining LIKE 'SAW Welder%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Mohammed Al-Qahtani', 'Production', 'Welder — SAW',
      ...
      'HIGH', '2025-08-15', 'IN_PROGRESS',
      'Enrolled in pre-qualification practice with Production Supervisor.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Khalid Al-Dossari' AND requiredTraining LIKE 'ISO 14001%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Khalid Al-Dossari', 'IMS', 'IMS Coordinator',
      ...
      'MEDIUM', '2025-10-31', 'OPEN',
      'Prepare for combined IMS surveillance audit scheduled Q4 2025.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Faisal Al-Ghamdi' AND requiredTraining LIKE 'Manufacturer tooling%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Faisal Al-Ghamdi', 'Production', 'Roll-Forming Operator',
      ...
      'MEDIUM', '2025-07-31', 'CLOSED',
      'Completed June 2025. Operator signed competency sign-off sheet.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Abdullah Al-Harbi' AND requiredTraining LIKE 'NEBOSH%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Abdullah Al-Harbi', 'Safety', 'HSE Officer',
      ...
      'HIGH', '2025-09-01', 'IN_PROGRESS',
      'Enrolled in NEBOSH online revision course. Examination booked for August 2025.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Omar Al-Otaibi' AND requiredTraining LIKE 'MBMA%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Omar Al-Otaibi', 'Design', 'Structural Engineer',
      ...
      'LOW', '2025-12-31', 'OPEN',
      'Scheduled for MBMA workshop in Q4 2025 when AISC hosts regional session.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Saleh Al-Mutairi' AND requiredTraining LIKE 'Supplier Auditing%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Saleh Al-Mutairi', 'Supply Chain', 'Procurement Officer',
      ...
      'MEDIUM', '2025-11-30', 'OPEN',
      'Required to take lead on next supplier audit cycle as per Q4 audit plan.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM HrTrainingNeed WHERE employeeName = 'Nasser Al-Shahrani' AND requiredTraining LIKE 'Ficep%') THEN
    INSERT INTO HrTrainingNeed (id, employeeName, department, roleTitle, competencyGap, requiredTraining, priority, targetDate, status, notes, updatedAt)
    VALUES (UUID(), 'Nasser Al-Shahrani', 'Production', 'CNC Operator',
      ...
      'MEDIUM', '2025-08-31', 'OPEN',
      'New Ficep line commissioned Q2 2025. Training to be arranged with OEM representative.', NOW());
  END IF;
END$$
DELIMITER ;
CALL seed_training_needs();
DROP PROCEDURE IF EXISTS seed_training_needs;

-- ──────────────────────────────────────────────────────────────────────────────
-- 4. Incidents / Near-Miss (FRM-024)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_incidents;
DELIMITER $$
CREATE PROCEDURE seed_incidents()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM ImsIncident WHERE incidentNumber = 'INC-2025-0001') THEN
    INSERT INTO ImsIncident (id, incidentNumber, title, incidentType, incidentDate, location, description, immediateAction, rootCause, correctiveAction, preventiveAction, severity, status, closedAt, updatedAt)
    VALUES (UUID(), 'INC-2025-0001', 'Fall from elevated work platform — fabrication bay',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsIncident WHERE incidentNumber = 'INC-2025-0002') THEN
    INSERT INTO ImsIncident (id, incidentNumber, title, incidentType, incidentDate, location, description, immediateAction, rootCause, correctiveAction, preventiveAction, severity, status, closedAt, updatedAt)
    VALUES (UUID(), 'INC-2025-0002', 'Near-miss: overhead crane hook swing close to worker',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsIncident WHERE incidentNumber = 'INC-2025-0003') THEN
    INSERT INTO ImsIncident (id, incidentNumber, title, incidentType, incidentDate, location, description, immediateAction, rootCause, correctiveAction, preventiveAction, severity, status, closedAt, updatedAt)
    VALUES (UUID(), 'INC-2025-0003', 'First aid: grinding disc fragment eye injury',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsIncident WHERE incidentNumber = 'INC-2025-0004') THEN
    INSERT INTO ImsIncident (id, incidentNumber, title, incidentType, incidentDate, location, description, immediateAction, rootCause, correctiveAction, preventiveAction, severity, status, closedAt, updatedAt)
    VALUES (UUID(), 'INC-2025-0004', 'Near-miss: hot metal weld spatter ignites cardboard packaging',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsIncident WHERE incidentNumber = 'INC-2025-0005') THEN
    INSERT INTO ImsIncident (id, incidentNumber, title, incidentType, incidentDate, location, description, immediateAction, rootCause, correctiveAction, preventiveAction, severity, status, updatedAt)
    VALUES (UUID(), 'INC-2025-0005', 'Dangerous occurrence: structural jig collapse during assembly',
...
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_drills;
DELIMITER $$
CREATE PROCEDURE seed_drills()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM ImsEmergencyDrill WHERE drillNumber = 'DRILL-2025-0001') THEN
    INSERT INTO ImsEmergencyDrill (id, drillNumber, drillType, scheduledDate, conductedDate, location, participantCount, objectives, findings, correctiveActions, status, notes, updatedAt)
    VALUES (UUID(), 'DRILL-2025-0001', 'FIRE_EVACUATION', '2025-01-20', '2025-01-20', 'Full Facility — all bays + offices', 147,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsEmergencyDrill WHERE drillNumber = 'DRILL-2025-0002') THEN
    INSERT INTO ImsEmergencyDrill (id, drillNumber, drillType, scheduledDate, conductedDate, location, participantCount, objectives, findings, correctiveActions, status, notes, updatedAt)
    VALUES (UUID(), 'DRILL-2025-0002', 'FIRST_AID', '2025-02-12', '2025-02-12', 'Fabrication Bay 2 — First Aid Station', 18,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsEmergencyDrill WHERE drillNumber = 'DRILL-2025-0003') THEN
    INSERT INTO ImsEmergencyDrill (id, drillNumber, drillType, scheduledDate, conductedDate, location, participantCount, objectives, status, notes, updatedAt)
    VALUES (UUID(), 'DRILL-2025-0003', 'FIRE_EVACUATION', '2025-07-15', '2025-07-15', 'Fabrication Bays 1 & 2 + Offices', 120,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsEmergencyDrill WHERE drillNumber = 'DRILL-2025-0004') THEN
    INSERT INTO ImsEmergencyDrill (id, drillNumber, drillType, scheduledDate, conductedDate, location, participantCount, objectives, findings, correctiveActions, status, notes, updatedAt)
    VALUES (UUID(), 'DRILL-2025-0004', 'CHEMICAL_SPILL', '2025-03-25', '2025-03-25', 'Coating & Paint Shop — Hazmat Storage Area', 22,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsEmergencyDrill WHERE drillNumber = 'DRILL-2025-0005') THEN
    INSERT INTO ImsEmergencyDrill (id, drillNumber, drillType, scheduledDate, conductedDate, location, participantCount, objectives, findings, correctiveActions, status, notes, updatedAt)
    VALUES (UUID(), 'DRILL-2025-0005', 'GENERAL', '2025-09-10', '2025-09-10', 'Night Shift — all fabrication bays', 62,
...
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_toolbox_talks;
DELIMITER $$
CREATE PROCEDURE seed_toolbox_talks()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0001') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, updatedAt)
    VALUES (UUID(), 'TBT-2025-0001', 'Safe Handling of Structural Steel Members — Slinging & Rigging',
      ...
      'COMPLETED', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0002') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, updatedAt)
    VALUES (UUID(), 'TBT-2025-0002', 'Welding Fume Control — Ventilation & RPE in Confined Spaces',
      ...
      'COMPLETED', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0003') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, notes, updatedAt)
    VALUES (UUID(), 'TBT-2025-0003', 'Working at Height — 100% Tie-Off Rule & Harness Inspection',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0004') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, notes, updatedAt)
    VALUES (UUID(), 'TBT-2025-0004', 'Hot Work Permit & Fire Watch Responsibilities',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0005') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, updatedAt)
    VALUES (UUID(), 'TBT-2025-0005', 'Manual Handling of Steel Components — RAMS Review',
      ...
      'COMPLETED', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0006') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, notes, updatedAt)
    VALUES (UUID(), 'TBT-2025-0006', 'Crane Exclusion Zones & Banksman Communication Signals',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0007') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, updatedAt)
    VALUES (UUID(), 'TBT-2025-0007', 'Shot Blast & PPE — Hearing & Respiratory Protection',
      ...
      'COMPLETED', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0008') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, notes, updatedAt)
    VALUES (UUID(), 'TBT-2025-0008', 'Pre-Shift Safety Brief: Rafter Assembly Campaign — Project HSG-277',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0009') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, notes, updatedAt)
    VALUES (UUID(), 'TBT-2025-0009', 'Summer Heat Stress Prevention Program',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM ImsToolboxTalk WHERE talkNumber = 'TBT-2025-0010') THEN
    INSERT INTO ImsToolboxTalk (id, talkNumber, topic, conductedDate, location, attendeeCount, durationMinutes, content, followUpActions, status, updatedAt)
    VALUES (UUID(), 'TBT-2025-0010', 'Compressed Gas Cylinder Safety — Storage, Handling & Connections',
...
END$$
DELIMITER ;
CALL seed_toolbox_talks();
DROP PROCEDURE IF EXISTS seed_toolbox_talks;

-- ──────────────────────────────────────────────────────────────────────────────
-- 7. Welder Qualification Records (FRM-017)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_welder_qualifications;
DELIMITER $$
CREATE PROCEDURE seed_welder_qualifications()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2024-0001') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2024-0001', 'Mohammed Al-Qahtani', 'WLR-041', 'SMAW', '3G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2024-0002') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2024-0002', 'Ahmed Al-Zahrani', 'WLR-019', 'GMAW', '2G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2024-0003') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, diameter, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2024-0003', 'Sulaiman Al-Dosari', 'WLR-055', 'GTAW', '6G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2024-0004') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2024-0004', 'Jaber Al-Khaldi', 'WLR-007', 'SAW', '1G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2024-0005') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2024-0005', 'Tariq Al-Asmari', 'WLR-033', 'FCAW', '3G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2023-0012') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, qualificationRange, certificationNumber, validFrom, expiryDate, renewalDate, notes, updatedAt)
    VALUES (UUID(), 'WQT-2023-0012', 'Rakan Al-Shehri', 'WLR-062', 'SMAW', '2G',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcWelderQualification WHERE wqtNumber = 'WQT-2025-0001') THEN
    INSERT INTO QcWelderQualification (id, wqtNumber, welderName, welderCode, weldingProcess, position, baseMaterial, fillMaterial, thickness, testDate, testLocation, visualResult, bendTestResult, radiographyResult, overallResult, notes, updatedAt)
    VALUES (UUID(), 'WQT-2025-0001', 'Hamad Al-Enezi', 'WLR-071', 'GMAW', '3G',
...
END$$
DELIMITER ;
CALL seed_welder_qualifications();
DROP PROCEDURE IF EXISTS seed_welder_qualifications;

-- ──────────────────────────────────────────────────────────────────────────────
-- 8. Coating Inspection Records (FRM-022)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_coating_inspections;
DELIMITER $$
CREATE PROCEDURE seed_coating_inspections()
BEGIN
  DECLARE v_projectId CHAR(36) DEFAULT NULL;
  SELECT id INTO v_projectId FROM Project WHERE projectNumber = '277' LIMIT 1;

  IF NOT EXISTS (SELECT 1 FROM QcCoatingInspection WHERE inspectionNumber = 'COAT-2025-0001') THEN
    INSERT INTO QcCoatingInspection (id, inspectionNumber, projectId, coatingSystem, coatLayer, surfacePrep, ambientTemp, relativeHumidity, dewPoint, nominalDft, minDft, maxDft, readings, averageDft, result, inspectionDate, witnessedBy, remarks, updatedAt)
    VALUES (UUID(), 'COAT-2025-0001', v_projectId, 'Jotun Barrier 80 Epoxy Primer', 'PRIMER',
      'Sa 2.5 per ISO 8501-1, profile Rz 50-75 µm', 28.0, 52.0, 18.0, 75, 60, 120,
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcCoatingInspection WHERE inspectionNumber = 'COAT-2025-0002') THEN
    INSERT INTO QcCoatingInspection (id, inspectionNumber, projectId, coatingSystem, coatLayer, surfacePrep, ambientTemp, relativeHumidity, dewPoint, nominalDft, minDft, maxDft, readings, averageDft, result, inspectionDate, witnessedBy, remarks, updatedAt)
    VALUES (UUID(), 'COAT-2025-0002', v_projectId, 'Jotun Hardtop AX Alkyd Topcoat (RAL 9002)', 'TOP_COAT',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcCoatingInspection WHERE inspectionNumber = 'COAT-2025-0003') THEN
    INSERT INTO QcCoatingInspection (id, inspectionNumber, projectId, coatingSystem, coatLayer, surfacePrep, ambientTemp, relativeHumidity, dewPoint, nominalDft, minDft, maxDft, readings, averageDft, result, inspectionDate, witnessedBy, remarks, updatedAt)
    VALUES (UUID(), 'COAT-2025-0003', v_projectId, 'Jotun Barrier 80 Epoxy Primer', 'PRIMER',
      ...
      'REJECTED — all 5 DFT readings below 60 µm minimum. Rework required: re-blast and re-apply primer.', NOW());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcCoatingInspection WHERE inspectionNumber = 'COAT-2025-0004') THEN
    INSERT INTO QcCoatingInspection (id, inspectionNumber, projectId, coatingSystem, coatLayer, surfacePrep, ambientTemp, relativeHumidity, dewPoint, nominalDft, minDft, maxDft, readings, averageDft, result, inspectionDate, witnessedBy, remarks, updatedAt)
    VALUES (UUID(), 'COAT-2025-0004', v_projectId, 'Jotun Barrier 80 Epoxy Primer — Rework (after COAT-2025-0003 rejection)', 'PRIMER',
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM QcCoatingInspection WHERE inspectionNumber = 'COAT-2025-0005') THEN
    INSERT INTO QcCoatingInspection (id, inspectionNumber, projectId, coatingSystem, coatLayer, surfacePrep, ambientTemp, relativeHumidity, dewPoint, nominalDft, minDft, maxDft, readings, averageDft, result, inspectionDate, witnessedBy, remarks, updatedAt)
    VALUES (UUID(), 'COAT-2025-0005', v_projectId, 'Interzone 954 — Epoxy Intermediate Coat (industrial zone specification)', 'MID_COAT',
...
END$$
DELIMITER ;
CALL seed_coating_inspections();
DROP PROCEDURE IF EXISTS seed_coating_inspections;

-- ──────────────────────────────────────────────────────────────────────────────
-- 9. Project Kickoff Checklists (FRM-016)
-- ──────────────────────────────────────────────────────────────────────────────
DROP PROCEDURE IF EXISTS seed_kickoff_checklists;
DELIMITER $$
CREATE PROCEDURE seed_kickoff_checklists()
BEGIN
  DECLARE v_proj1 CHAR(36) DEFAULT NULL;
  DECLARE v_proj2 CHAR(36) DEFAULT NULL;
  SELECT id INTO v_proj1 FROM Project WHERE projectNumber = '277' LIMIT 1;
  SELECT id INTO v_proj2 FROM Project WHERE projectNumber = '257' LIMIT 1;

  IF v_proj1 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM ProjectKickoffChecklist WHERE projectId = v_proj1) THEN
    ...
    VALUES (UUID(), v_proj1, '2025-01-08 09:00:00', 'Hexa Steel HQ — Conference Room A',
  ...
  IF v_proj2 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM ProjectKickoffChecklist WHERE projectId = v_proj2) THEN
    ...
    VALUES (UUID(), v_proj2, '2025-02-17 10:00:00', 'Hexa Steel HQ — Board Room',
