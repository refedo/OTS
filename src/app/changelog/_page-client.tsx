...
const hardcodedVersions: ChangelogVersion[] = [
  {
    ...
    highlights: [
      'Saudi Labor Law unauthorized-absence alerts & escalation (OTS-BL-080): automated HR compliance escalation — HR → manager → CEO — with override flags.',
      ...
      'Project Tracker: tonnage aggregated in SQL; in-flight deduplication and 60s cache; null-building tasks excluded.',
    ],
    changes: {
      added: [
        'Saudi Labor Law unauthorized-absence alerts (OTS-BL-080): automated escalation workflow for uncovered employee absences with HR → manager → CEO notification chain, override flags, and audit trail.',
        'Aging Report: minimum-amount filter to exclude small receivable balances from the aged view.',
        'Projects: wizard edit mode — existing projects can be re-entered into the creation wizard for editing.',
        'Projects: new Review step in the project creation/edit wizard before saving.',
        'Projects: redesigned project details page.',
      ],
      fixed: [
        'OOM restart loop — root-cause diagnostics: memory sampled every 5s with persisted peak RSS; SIGINT/SIGHUP handled; restarts classified by peak memory; heap snapshot on threshold; V8 near-heap-limit backstop.',
        'OOM restart loop — culprit attribution: per-process counters persisted into 5s state file; requests tracked in-flight; Prisma queries counted by shape.',
        'PM2 memory limits raised to match the actual 7.8GB host — prior limit was too low.',
        'Heap snapshot watchdog: heapsnapshots/ directory is now bounded so it cannot fill the host disk.',
        ...
        'Restart death spiral eliminated by capping thresholds and removing memory-exhausting polling.',
        'Tasks: Discussion-type tasks now visible in the task list.',
        'Tasks: isCeoTask NULL silent filter removed — tasks with no CEO flag now appear.',
        'Tasks: scope dropdown now visible in edit mode; API response keys aligned with form.',
        'Tasks: filter dropdowns open upward to prevent clipping below the viewport.',
        'Tasks: SearchableSelect max-height increased; uses React portal to escape stacking context.',
        'Tasks: createPortal import fixed — was imported from react instead of react-dom.',
        'Project Tracker: tonnage aggregated in SQL instead of loading all rows (eliminates N+1 full-table scan).',
        'Project Tracker: in-flight deduplication and 60s cache prevent thundering-herd on tracker API.',
        'Project Tracker: null-building tasks excluded from per-building progress percentage.',
        'Inventory: missing unit_cost / total_cost columns repaired on inv_stock_ledger.',
        'Inventory: reference_no migration added so stock ledger cost migration succeeds.',
        'Inventory: camelCase column name fix in stock ledger migrations.',
        'Build: server-only Prisma client chain no longer leaks into the client bundle.',
        'Polling: underperforming-schedules query moved from every 60s to once per day.',
        'Polling: underperforming-schedules removed from global notification polling loop.',
      ],
      changed: [],
    },
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Inventory & MIR: Pricing, Quarantine, Category Fix, Purchases History & Pagination',
    highlights: [
      'Material pricing in stock: unit price captured from PO line at MIR receipt and flows through to inv_stock_ledger as unit_cost / total_cost.',
      'Purchases tab in Material Master detail panel: full stock-in history with date, MIR ref, supplier, qty, costs, and weighted average price per tonne.',
      'Quarantine Qty field in MIR inspection form: auto-computes rejected = received − accepted − quarantine.',
      'Receive All / Accept All bulk action buttons in MIR receipt detail view (Draft workflow).',
      ...
      'Sidebar active state fixed: /inv, /hr, and /backlog/dashboard no longer stay highlighted on sub-pages.',
    ],
    changes: {
      added: [
        'Material pricing in stock: unit_cost and total_cost captured from PO line at MIR receipt creation, stored in inv_stock_ledger for weighted average cost calculation.',
        'Purchases tab in Material Master detail panel: stock-in movements with date, MIR ref, supplier, qty, unit cost, total cost, weighted avg price, avg price per tonne.',
        'Quarantine Qty field in MIR item inspection form: auto-computes rejected = received − accepted − quarantine.',
      ...
      fixed: [
        'Stock Levels page showed "Other" category for structural steel items (HEA, IPE, HEB profiles) — corrected by switching sync-items to use material_category via mapMaterialCategory.',
      ],
      changed: [
        'Category fix: sync-items now uses material_category (via mapMaterialCategory) instead of item_class — existing items with stale category updated on every sync run.',
  ...
  },
  {
    ...
    changes: {
      added: [
        ...
        'Stage backfill on COMPLETED: skipped intermediate stages (Under Review, Approved, Planned, In Progress) stamped with finisher name + timestamp.',
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Backlog Dashboard — Contributors, Domains & Progress',
    highlights: [
      'New Backlog Dashboard at /backlog/dashboard with live KPI strip, Top Contributors leaderboard, Development Types panel, and Domain Categories grid.',
      'Top Contributors ranked with gold/silver/bronze medals, contribution bars, and per-contributor completion rate.',
      '13 domain categories as colour-coded cards linking to filtered backlog board.',
      'Priority Distribution breakdown with percentage-of-total bars; Add Backlog quick-add dialog inline.',
      'Auto-refresh every 2 minutes with manual Refresh button.',
    ],
    changes: {
      added: [
        'Backlog Dashboard (/backlog/dashboard) — KPI strip: total items, completed, in progress, blocked, pipeline, active contributors.',
        'Top Contributors leaderboard — ranked by backlog items created, gold/silver/bronze medals, relative contribution bar, per-contributor completion rate.',
        'Development Types panel — all 8 backlog types with type icons and completion progress bars.',
        'Domain Categories grid — 13 system domains as colour-coded clickable cards with item counts, completion bars, and active indicators; each links to filtered backlog board.',
        'Priority Distribution — Critical / High / Medium / Low breakdown with percentage bars, each linking to filtered backlog.',
        'Add Backlog inline dialog — quick-add form (title, description, type, category, priority) without leaving the dashboard.',
        '/api/backlog/dashboard GET endpoint — aggregates contributor stats, category/type/priority breakdowns, and overall KPIs.',
      ],
      fixed: [],
      changed: [
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'HR Turnover Rate KPI',
    highlights: [
      'New Employee Turnover Rate KPI section on the HR Dashboard.',
      'Shows headcount at start/end, new hires, leavers, and auto-calculated turnover rate for any selected period.',
      'Colour-coded stability indicator: Good Stability (<10%), Normal (10–20%), Requires Further Analysis (>20%).',
      'All figures respect existing occupation / section / department filters.',
    ],
    changes: {
      added: [
        'Employee Turnover Rate KPI on HR Dashboard: At Start, At End, New Hires, Leavers, and Turnover Rate (auto-calculated as Leavers ÷ Average Headcount × 100).',
        'Stability indicator: colour-coded and labelled by threshold — < 10% Good Stability (emerald), 10–20% Normal (amber), > 20% Requires Further Analysis (rose).',
      ],
      fixed: [],
      changed: [
        'All turnover figures respect existing occupation / section / department filters and refresh on date range change.',
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'IMS PDF Downloads, Seed Data & Global Search Expansion',
    highlights: [
      'PDF download on all IMS reports: Audit Plans (FRM-006/007/008), Management Review MOM + Report (FRM-011/012), Processes list (FRM-002/004), and Objectives (FRM-013).',
      'IMS seed data: AP-25-001/002, AP-26-001, NCR-25-001 through NCR-26-001, MR-25-001/MR-26-001 management reviews, 5 company objectives for 2026.',
      'Global search expanded to cover IMS audit plans, management reviews, NCRs, QMS processes, approved suppliers, objectives, and BD companies.',
      'Steel theme (#2c3e50) added to PDFReportBuilder and applied across all IMS hero sections.',
    ],
    changes: {
      added: [
        'ims-pdf-generator.ts: unified PDF generation for all IMS reports with company logo (loadLogoBase64()) and steel theme.',
        'Audit Plans (/ims/audit-plans): per-row and detail-page PDF buttons generating FRM-006/007 schedule + FRM-008 NCR findings section.',
        'Management Review: separate FRM-011 MOM and FRM-012 Report PDF download buttons.',
        'Processes list (/ims/processes): landscape FRM-002/004 PDF download.',
        'Objectives (/business-planning/objectives): FRM-013 PDF grouped by category with key result sub-table.',
        'Steel theme in pdf-builder.ts (#2c3e50).',
        'Global search covers IMS audit plans, management reviews, audit findings/NCRs, QMS processes, approved suppliers, company objectives, and BD companies.',
        'Seed data (seed_sprint_2280_data.sql): AP-25-001/002, AP-26-001, NCR-25-001 through NCR-26-001, MR-25-001/MR-26-001, 5 company objectives for 2026 with key results.',
        'startup-migrations.ts: registered seed_sprint_2280_data.sql.',
      ],
      fixed: [],
      changed: [
        'Gradient heroes (#2c3e50) + emoji KPI strips applied to all updated pages.',
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Missing Forms Sprint — Safety & HSE, QMS Processes, Supplier List & More',
    highlights: [
      '9 new Prisma models, 7 SQL migrations, 16 API route files, and 14 page components in one sprint.',
      'New pages: QMS Process List (FRM-002/004), Incidents/Near-Miss (FRM-024), Emergency Drills (FRM-025), Toolbox Talks (FRM-026), Approved Supplier List (FRM-003), Kickoff Meeting Checklist (FRM-016), Welder Qualification (FRM-017), Coating Inspection DFT (FRM-022), and TNA tab in /hr/training (FRM-005).',
      'IsoClauseNote banner added to all 13 IMS pages plus QC Welding and Objectives.',
      '8 new sidebar entries; Quick Guide updated with Safety & HSE and QMS sections.',
    ],
    changes: {
      added: [
        '/ims/processes — QMS Process List (FRM-002/004): list all processes with owner, KPI, risks, and audit frequency.',
        '/ims/safety/incidents — Incidents/Near-Miss Register (FRM-024).',
        '/ims/safety/drills — Emergency Drills Register (FRM-025).',
        '/ims/safety/toolbox-talks — Toolbox Talks Log (FRM-026).',
        '/supply-chain/approved-suppliers — Approved Supplier List (FRM-003).',
        '/projects/kickoff — Kickoff Meeting Checklist (FRM-016).',
        '/qc/welder-qualification — Welder Qualification Register (FRM-017).',
        '/qc/coating — Coating Inspection DFT Log (FRM-022).',
        'TNA second tab in /hr/training — Training Needs Analysis (FRM-005).',
        'IsoClauseNote reusable banner added to all 13 IMS pages + QC welding + Objectives.',
        '8 new sidebar entries across Safety & HSE, Supply Chain, Projects, and QC sections.',
        'Quick Guide sections: 08 Safety & HSE and 09 QMS Process List.',
      ],
      fixed: [],
      changed: [
        'Version bumped to 22.7.0',
      ],
    },
  },
  {
    ...
    type: 'patch',
    status: 'previous',
    mainTitle: 'IMS Audit Compliance — HEXA-FRM References & Management Review Expansion',
    highlights: [
      'HEXA-FRM / Hexa-ISP / ISO clause reference badges added to all IMS and QC module hero sections — auditors can now trace every screen to its governing form.',
      'Management Review (HEXA-FRM-011) expanded: Attendees section (§9.3.1) and all 12 ISO-required §9.3.2 inputs — 5 auto-populated from live OTS data, 7 editable text fields.',
      '5 new schema fields on ImsManagementReview + migration.',
    ],
    changes: {
      added: [
        'HEXA-FRM / Hexa-ISP / ISO clause badges on all IMS and QC hero sections: change-requests, documents, audit-plans, management-review, risks, legal-register, competence-matrix, ITP, qc/material, qc/dimensional, qc/ncr.',
        'Attendees section in Management Review (ISO §9.3.1 — top management participation evidence).',
        '§9.3.2 inputs expanded from 6 to all 12 ISO-required items; 5 auto-populated (NCRs, KPIs, audit findings, OH&S risks, previous review decisions); 7 editable text fields.',
        '5 new schema fields: inputPreviousActions, inputContextChanges, inputDesignPerformance, inputOhsPerformance, inputEnvironmentalPerf.',
        'Migration: ims_management_review_iso_inputs.sql.',
        'Populate route enhanced: 8 parallel queries including real ImsAuditFinding data and previous review auto-fill.',
      ],
      fixed: [],
      changed: [
        'Version bumped to 22.6.4',
      ],
    },
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'CEO HR Letter Edit/Cancel & Purpose Field Fix',
    highlights: [
      'CEO users can now edit APPROVED letters and cancel them with an optional reason — previously letters were locked after approval.',
      'Runtime crash fixed: purpose field was missing from the Letter type and openEdit(), causing .trim() on undefined.',
      'purpose now persists on PUT via updated Zod schema.',
      'CANCELLED added to HrLetterStatus enum + DB migration.',
    ],
    changes: {
      added: [
        'POST /api/hr/letters/[id]/cancel — CEO-only endpoint that sets letter status to CANCELLED with optional reason.',
        'Cancel (Ban icon) and Edit (Pencil icon) action buttons for APPROVED letters — visible to CEO users only.',
        'Cancel confirmation dialog with optional reason text field.',
        'CANCELLED status to HrLetterStatus enum + DB migration (add_letter_cancelled_status.sql).',
      ],
      fixed: [
        'Runtime crash when editing letters: purpose missing from Letter type and openEdit() — caused .trim() on undefined.',
        'purpose field not persisting on PUT: added to updateSchema Zod and Prisma update spread.',
        'Non-CEO edit button hidden for APPROVED letters (CEO edit button shown instead).',
      ],
      changed: [
        'CEO (hr.letters.approveCeo) can now edit APPROVED letters via PUT.',
  ...
  },
  {
    ...
    type: 'patch',
    status: 'previous',
    mainTitle: 'Stability Bump — Account Invoice Report & Orphan Purge Confirmed',
    highlights: [
  ...
  },
  {
    ...
    type: 'patch',
    status: 'previous',
    mainTitle: 'PO–Invoice Linkage, COA Credit Balance, Date Fix & Dashboard Reports',
    highlights: [
      'New PO–Invoice Linkage Report: groups supplier invoices with their POs by supplier + project, showing variance indicators (over-invoiced / pending).',
      'New COA Credit Balance Report: full chart of accounts with opening, period, and closing debit/credit columns grouped by account type.',
      'Hijri date bug fixed: all en-SA locale calls for dates updated to en-SA-u-ca-gregory to force Gregorian calendar across all modules.',
      'Both new reports added to the sidebar and financial dashboard reports grid.',
    ],
    changes: {
      added: [
        '/financial/reports/po-invoice-linkage — groups supplier invoices with their purchase orders by supplier + project; each group shows both PO table and invoice table side-by-side with a variance indicator (over-invoiced / pending invoices); filters by supplier name and invoice date range.',
        '/financial/reports/coa-credit-balance — full chart of accounts with opening debit/credit, period debit/credit, and closing debit/credit columns; grouped by account type (Assets, Liabilities, Equity, Revenue, Expenses, COGS); live search, expand/collapse all; grand totals footer.',
  ...
  },
  {
    ...
    highlights: [
      ...
      'Three operating modes: READ_ONLY (observe and report), ANNOTATE (flag records with risk notes), and FULL_ACTOR (trigger escalations and create follow-up tasks). Mode is enforced at the API harness level before any write reaches the database.',
      'Separate OpsRiskFlag table lets the agent annotate individual entities without touching the existing EWS — a side-by-side comparison banner surfaces both views together.',
      'Automatic daily cron on the Saudi work week (Saturday–Wednesday, 07:00 Riyadh). Trigger manually from the /ops-agent dashboard at any time with a single click.',
      'Full UI dashboard at /ops-agent: live Ops Brief, risk flag resolution, mode switcher with FULL_ACTOR confirmation, configurable thresholds, and the last 10 run history.',
      'Push notifications dispatched to all ops_agent.view users after each completed run when RED or AMBER signals are detected.',
    ],
    changes: {
      added: [
        'OpsAgentConfig model: mode (READ_ONLY/ANNOTATE/FULL_ACTOR), thresholds (taskStaleDays, projectStaleDays, otApprovalHours), enabled modules, schedule config — seeded on boot',
        'OpsAgentRun model: run status, structured brief JSON, input/output tokens, durationMs, Claude session ID, trigger type (cron/manual/event)',
        'OpsRiskFlag model: entity-level flags with severity (RED/AMBER/GREEN), entity type/label, agent note, module tag, resolution tracking',
        'Idempotent SQL migration: prisma/manual_migrations/add_ops_agent_module.sql',
        'Anthropic SDK client with managed-agents-2026-04-01 beta header',
        '9 agent tool definitions — read: get_stale_tasks, get_project_health, get_pipeline_stalls, get_hr_flags, get_project_status, get_recent_system_events; write (mode-gated): flag_record, create_followup_task, trigger_escalation',
        'Agent harness: mode enforcement, tool→endpoint routing, x-ots-agent-secret auth',
        'Full session loop: stream events → handle agent.custom_tool_use → send user.custom_tool_result → parse structured brief',
        'Internal agent routes (/api/agent/): tasks/stale, projects/health, pipeline/stalls, hr/flags, projects/status, events/recent, actions/flag-record, actions/create-task, actions/trigger-escalation',
        'Management API: POST /api/ops-agent/run (202 fire-and-forget), GET /run/[id], GET /runs, GET+PATCH /config, POST /cron, PATCH /flags/[id]/resolve',
        'OpsAgentScheduler: Sat–Wed 07:00 Riyadh; ENABLE_OPS_AGENT_SCHEDULER env var; registered in cron-registry',
        'ops_agent.* permission category: view, run, configure, resolve_flags',
        '/ops-agent navigation guard in navigation-permissions.ts',
        '10 new event types under OPS_AGENT category in system-events',
        '/ops-agent page: OpsAgentLayout (live polling), RunTriggerCard, OpsBriefView (RED/AMBER/GREEN cards), RiskFlagList (with Resolve), RunHistoryList, ModeSwitcher, ThresholdEditor, ComparisonBanner',
        'Ops Agent sidebar entry: Radar icon, newSince badge, between CEO Dashboard and Early Warning',
        'Push notifications to ops_agent.view users after runs with RED/AMBER flags',
  ...
  },
  {
    ...
    highlights: [
      ...
      'Attendance page consolidated: Records, Mapping, and Timesheet are now tabs on the same /hr/attendance page — Attendance Mapping and Employee Timesheet removed from sidebar.',
      'Global search expanded to cover HR entities: Employees, Assets, Contracts, and HR Letters — all searchable from the Ctrl+K palette.',
      '+ Quick Task button added to the top bar (also Ctrl+Shift+T): opens a dialog to create a task with title, priority, and due date without leaving any page.',
      'Startup migrations now auto-run add_hr_letters.sql on server boot; agencies page marked force-dynamic to fix intermittent refresh loop.',
    ],
    changes: {
      added: [
        'POST /api/hr/payroll-periods/[id]/unapprove: reverts APPROVED → CALCULATED; requires hr.payroll.approve; blocked on LOCKED/PAID periods',
        '"Revert Approval" amber-outlined button in payroll period detail (visible only when status === APPROVED and canApprove)',
        'AttendanceTabsClient: tabbed wrapper with Records | Mapping (with unmapped badge count) | Timesheet tabs at /hr/attendance',
        'Timesheet tab shows a launch card linking to /hr/attendance/timesheet/EMPLOYEE/{first}',
        'QuickTaskDialog in TopBar: + button opens dialog with title, priority (Low/Medium/High/Critical), optional due date; submits to POST /api/tasks',
        'Ctrl+Shift+T global shortcut opens Quick Task dialog from anywhere',
        'Global search: employees (name, ID, occupation), assets (code, name, plate, serial), contracts (title, number, reference), hr letters (number, subject) added to /api/search and GlobalSearch component',
        'add_hr_letters.sql added to STARTUP_MIGRATIONS list in startup-migrations.ts',
      ],
      fixed: [
        'Agencies page: added export const dynamic = "force-dynamic" to prevent stale static rendering causing apparent refresh loop',
        'Payroll periods list now surfaces non-OK API error messages instead of silent empty state',
      ],
      changed: [
        '"Employee Timesheet" and "Attendance Mapping" sidebar entries removed — content accessible via /hr/attendance tabs',
        'GlobalSearch: SearchResults interface extended with employees, assets, contracts, hrLetters; CATEGORY_META updated with new icons',
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Unified Asset Assignment Log + Contracts & Documents Management',
    highlights: [
      '/hr/assets gains a tabbed layout with a new "Assignment Log" tab showing every assign/return transaction across all assets, filterable by status, category, and search.',
      'New /hr/contracts page manages company contracts (health insurance, medical, Iqamas, car registrations, vehicle licenses, professional licenses, commercial registration, legal documents, and other) with Hijri date support, expiry color-coding, and daily reminder cron.',
      'Hijri↔Gregorian live conversion in create/edit dialogs — OTS uses Umm al-Qura algorithm, no npm package, accurate for 1400–1500 AH range.',
      'HR dashboard gains a "Contracts & Documents" 4-tile KPI row (Total Active, Expiring in 7d, Expiring in 30d, Expired) with clickable tiles.',
      'Two new permissions: hr.contracts.view + hr.contracts.manage. Auto-number CNT-YY-NNNN. Daily cron at 08:00 Riyadh sends DEADLINE_WARNING to all hr.contracts.manage users.',
    ],
    changes: {
      added: [
        'prisma/schema.prisma: Contract model with ContractType + ContractStatus enums; expiryDateHijri string field; notifyDaysBefore (default 30)',
        'prisma/manual_migrations/add_contracts.sql: idempotent CREATE TABLE IF NOT EXISTS with FK constraints and indexes',
        'src/lib/utils/hijri.ts: gregorianToHijri() + hijriToGregorian() using Umm al-Qura algorithm',
        'GET/POST /api/hr/contracts + GET/PUT/DELETE /api/hr/contracts/[id]',
        'POST /api/cron/contract-reminders: daily DEADLINE_WARNING deduped per contract per day',
        'ContractRemindersScheduler: daily at 08:00 Asia/Riyadh; ENABLE_CONTRACT_REMINDERS_SCHEDULER env var',
        '/hr/contracts page: amber/orange gradient hero, 4 KPI tiles, filterable table with Hijri + Gregorian expiry, color-coded days-left, create/edit/delete dialogs',
        '"Contracts & Docs" sidebar entry + entityRoutes push routing',
        'Asset-assignments API extended: status, category, search filters without requiring employeeId',
        'Assignment Log tab on /hr/assets with unified table, status/category filter bar',
        'hr.contracts.view + hr.contracts.manage permissions added to HR role bundle',
      ],
      fixed: [],
      changed: [
        'HR dashboard: Contracts & Documents 4-tile row added',
        'contracts → /hr/contracts added to push entityRoutes',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Asset Management — Cars, SIM Cards, Laptops, Violations & Car Maintenance',
    highlights: [
      'Full company asset registry covering cars, SIM cards, laptops, keys, tools, and equipment — with per-employee assignment timelines tracking open/return dates.',
      'Traffic violations & infractions per employee with payroll deduction flagging — violations are linked to the car asset and can be marked for automatic deduction in the next payroll run.',
      'Comprehensive car maintenance sheet: oil changes, brakes, tires, AC, odometer, service center, invoice attachment, and next-service reminder with color-coded overdue alerts.',
      'Employee detail page gains an "Assets" tab showing the full assignment timeline with color-coded current/returned rows and the employee\'s violation history with inline status updates.',
      'Six new permissions: hr.assets.{view,manage}, hr.violations.{view,manage}, hr.carMaintenance.{view,manage}. Three new HR pages: /hr/assets, /hr/traffic-violations, /hr/car-maintenance.',
    ],
    changes: {
      added: [
        'prisma/schema.prisma: Asset, AssetAssignment, TrafficViolation, CarMaintenanceRecord models + AssetCategory, AssetStatus, ViolationStatus, MaintenanceType enums',
        'prisma/manual_migrations/add_asset_management.sql: idempotent CREATE TABLE IF NOT EXISTS for all four tables',
        'GET/POST /api/hr/assets + GET/PUT/DELETE /api/hr/assets/[id]',
        'POST /api/hr/assets/[id]/assign + POST /api/hr/assets/[id]/return',
        'GET /api/hr/asset-assignments (with optional employeeId filter)',
        'GET/POST /api/hr/traffic-violations + GET/PUT/DELETE /api/hr/traffic-violations/[id]',
        'GET/POST /api/hr/car-maintenance + GET/PUT/DELETE /api/hr/car-maintenance/[id]',
        '/hr/assets: violet hero, KPI strip, asset cards with category icons, assign/return dialogs',
        '/hr/traffic-violations: rose hero, KPI strip, tabular list, payroll deduction flag',
        '/hr/car-maintenance: emerald hero, expandable service log cards with next-service reminders',
        'Employee detail "Assets" tab: full assignment timeline + violation history with inline status updates',
        'hr.assets.{view,manage}, hr.violations.{view,manage}, hr.carMaintenance.{view,manage} permissions',
      ],
      fixed: [],
      changed: [
        'HR sidebar: Assets, Traffic Violations, Car Maintenance entries added',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    type: 'minor',
    status: 'previous',
    mainTitle: 'Payroll Engine Phase 3 — Loans, Custodies & WPS SIF',
    highlights: [
      'Employee loans with monthly installment deductions: tracks principal, installment amount, total/paid count, and a year-warning flag (required when installmentsTotal > 12). Payroll auto-deducts and advances installmentsPaid on each approval cycle.',
      'Custody cash advances: per-custody deductionAmount that Finance sets to control per-payroll recovery, capped at outstanding balance. Auto-settles when fully recovered.',
      'WPS SIF generator (SAMA format): pipe-delimited EH header + ED records per employee, alongside the existing Alinma CSV. GET validation endpoint reports missing IBANs / National IDs before generation.',
      'Employee detail page gains a "Finance" tab with KPI tiles for loan count, loan balance, custody count, outstanding balance — plus vertical cards for each loan (progress bar + cancel) and custody (inline deduction-amount editor).',
      'Four new permissions: hr.loans.{view,manage} and hr.custodies.{view,manage} — all added to HR role bundle.',
    ],
    changes: {
      added: [
        'prisma/schema.prisma: Loan + Custody models with LoanStatus / CustodyStatus enums',
        'prisma/manual_migrations/add_loans_custodies.sql: CREATE TABLE IF NOT EXISTS for both tables + stored-procedure to add loanDeduction + custodyDeduction columns to PayrollLine',
        'GET/POST /api/hr/employees/[id]/loans + GET/PUT/DELETE /api/hr/employees/[id]/loans/[loanId]',
        'GET/POST /api/hr/employees/[id]/custodies + GET/PUT/DELETE /api/hr/employees/[id]/custodies/[custodyId]',
        'POST /api/hr/payroll-periods/[id]/wps-sif: generate SAMA WPS SIF file',
        'GET /api/hr/payroll-periods/[id]/wps-sif: validation report (missing IBANs / National IDs)',
        'src/lib/services/hr/wps-sif-generator.ts: WPS SIF generation service',
        'WPS_EMPLOYER_ID + WPS_BANK_ID env vars in src/lib/env.ts',
        'Employee Finance tab: KPI tiles + loan/custody cards with progress bars and inline editors',
        'hr.loans.{view,manage} + hr.custodies.{view,manage} permissions',
      ],
      fixed: [],
      changed: [
        'Payroll calculator: loanDeduction + custodyDeduction added to totalDeductions and persisted on PayrollLine',
        'Payroll approval (CALCULATED → APPROVED): advances installmentsPaid on loans, settles custody deductionAmount, auto-completes loans and custodies when fully paid',
        'Payslip PDF: conditional Loan/Custody deduction rows',
        '"WPS SIF (SAMA)" button added to payroll period detail (alongside existing Alinma CSV)',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    highlights: [
      ...
      'Payroll lifecycle: DRAFT → CALCULATED → APPROVED → LOCKED with configurable daily-rate basis (THIRTY / WORKING_DAYS_IN_MONTH / WEEKLY_AVERAGE), 1.5× overtime, per-line adjustments.',
      'GOSI handling: new Employee.isGosiSubject + gosiSalary columns so GOSI can run on a contract figure different from the basic, surfaced in the employee form and preserved across Dolibarr sync.',
      'WPS export: Alinma Bank SAMA-compliant CSV generator under public/outputs/wps/, downloadable per period.',
      'Payslip PDFs: per-employee A4 payslips via jsPDF + jspdf-autotable, saved under public/outputs/payslips/<periodId>/ and downloadable from the period detail page.',
      'End-of-Service gratuity: KSA labour-law calculator (0.5 mo/yr first 5 years, 1 mo/yr thereafter, prorated) with dedicated awards table.',
      'HR Setup: new Leave Types and Payroll Settings tabs; sidebar gets Leaves + Payroll entries under HR.',
    ],
    changes: {
      added: [
        'Schema: LeaveType, LeaveRequest, LeaveBalance, PayrollPeriod, PayrollLine, PayrollAdjustment, WpsExport, EndOfServiceAward + supporting enums',
        'Schema: Employee.isGosiSubject (Boolean, default true) and Employee.gosiSalary (Decimal, nullable)',
        'Manual migration prisma/manual_migrations/add_hr_phase_3.sql — idempotent DDL for all Phase 3 tables, seeds six default leave types and seven SystemConfig defaults (daily-rate basis, GOSI rates, overtime multiplier, WPS bank, approval chain, carry-over cap, default accrual)',
        '18 new hr.* permissions covering leaves.request/viewAll/approve/manageTypes, payroll.view/calculate/approve/lock/export/settings and eos.calculate',
        'Services: leave-balance-calculator (opening + accrued − used + adjustments, working days exclude Fridays + PublicHoliday), payroll-calculator (per-line compensation snapshot, leave class counting, overtime, GOSI, adjustments), wps-alinma-generator (SAMA CSV with SAL/TRL records), payslip-pdf-generator (A4 per employee), end-of-service-calculator (KSA formula, prorated years), system-config typed getters/setters',
        'API: /api/hr/leave-types (GET/POST) + [id] (PUT/DELETE soft archive)',
        'API: /api/hr/leave-requests (GET scope=mine/inbox/all, POST with working-days + balance snapshot + over-balance flag) + [id] (GET/PUT/DELETE) + [id]/action (submit/approve/reject with configurable stage chain)',
        'API: /api/hr/leave-balances (GET snapshots per employee/year)',
        'API: /api/hr/payroll-periods (GET/POST unique year/month) + [id] (GET/DELETE guard) + [id]/calculate + [id]/approve + [id]/lock + [id]/wps + [id]/payslips',
        'API: /api/hr/payroll-adjustments (GET/POST — creation flips CALCULATED/APPROVED period back to DRAFT)',
        'API: /api/hr/end-of-service (GET/POST upsert)',
        'API: /api/hr/settings (GET any auth / PUT gated by hr.payroll.settings)',
        'UI: /hr/leaves tabbed page (My leaves / Inbox / All) with balances grid, new request form, over-balance advance warning, and approve/reject actions per stage',
        'UI: /hr/payroll period list + create modal, /hr/payroll/[id] detail page with KPI cards, action row (Calculate / Approve / Lock / WPS / Payslips), WPS exports list and per-employee payslip PDF downloads',
        'UI: /hr/setup Leave Types and Payroll Settings tabs, both gated by their respective permissions',
        'Employee form: GOSI subject checkbox + GOSI salary input in the Compensation tab',
        'Sidebar: Leaves (CalendarClock) and Payroll (Banknote) entries under the HR group',
      ],
      fixed: [],
      changed: [
        'Employee create + update APIs: gosiSalary + isGosiSubject in createSchema/updateSchema, Decimal conversion via String(), gosiSalary added to TRACKED_SYNC_FIELDS so manual edits survive Dolibarr sync',
        'Payroll adjustment writes automatically flip a CALCULATED/APPROVED period back to DRAFT so stale calculations cannot reach the WPS export',
        'WPS generator intentionally skips employees without IBAN and logs a warning (Alinma rejects files with missing IBANs)',
        'Version bumped to 18.5.0',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    highlights: [
      ...
      'Participant management: add and remove people from task conversations with a full management panel',
      'Points leaderboard page at /points — full ranking table with streaks, badges, and your own position highlighted',
      'Building name now shown in task detail sidebar alongside the project',
      'Tonnage fallback improved — now distributes project-level assembly weights across buildings when building-specific data is missing',
      'Backlog 500 fixed — explicit column selection avoids crash when notes migration is pending',
    ],
    changes: {
      added: [
        'GET /api/points/me — lightweight endpoint for TopBar points badge',
        '/points leaderboard page with ranking table, streaks, badge count, and current user highlighted',
        'Building card in task details sidebar (designation + name)',
        'Participant management panel in conversations — invite by name search, remove with one click',
      ],
      fixed: [
        'Task messages now notify task assignee + creator automatically, not just existing participants',
        'Backlog API: explicit select avoids crash when ProductBacklogItem.notes column is missing',
        'Tonnage: fallback now queries all project assembly parts when building-level data is empty',
        'add_integration_toggles.sql: removed ADD COLUMN IF NOT EXISTS syntax',
        'add_task_conversations.sql: corrected ALTER TABLE `Notification` → `notifications`',
      ],
      changed: [
        'Early Warning sidebar label: removed ⚡ emoji',
        'Points badge added to TopBar upper-right (amber star, total points, rank)',
      ],
    },
  },
  {
    ...
    changes: {
      added: [
        'Aging Report: 6-bucket summary row (Current, 1-30, 31-60, 61-90, 90+, Total) with color-coded per-supplier detail table',
        'SOA: Overdue Balance card (past-due invoices only) + Total Outstanding card (all unpaid)',
        'SOA: Credit Limit card with headroom / over-limit indicator',
        'Balance Sheet: year selector dropdown alongside custom date input',
        'Financial Settings: stale account code detection with amber warnings and re-sync guidance',
        'Backlog: Eye icon opens images and PDFs inline; download kept as secondary action',
        'Deploy workflow: conditional npm ci, prisma generate, pm2 reload for zero-downtime deploys',
      ],
      fixed: [],
      changed: [
        'Aging Report layout: switched to single unified page with AR/AP toggle',
        'SOA totals section restructured to separate past-due from total outstanding',
      ],
    },
  },
  {
