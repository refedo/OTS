...
import { AddEventDialog } from '@/components/operations/AddEventDialog';
...
export default function EventManagementPage() {
  ...
  const [selectedProjectForDialog, setSelectedProjectForDialog] = useState<string | null>(null);
  const [buildingsForDialog, setBuildingsForDialog] = useState<Building[]>([]);
  ...
  const handleAddEvent = async (projectId: string) => {
    // Fetch buildings for the selected project
    try {
      const res = await fetch(`/api/buildings?projectId=${projectId}`);
      if (res.ok) {
        const data = await res.json();
        setBuildingsForDialog(data);
      }
    } catch (error) {
      console.error('Error fetching buildings:', error);
    }
    
    setSelectedProjectForDialog(projectId);
    setShowDialog(true);
  };
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Select Project to Add Event</Label>
              <Select onValueChange={(value) => handleAddEvent(value)}>
                <SelectTrigger>
            ...
            <p className="text-sm text-muted-foreground">
              Select a project above to open the event creation dialog
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {/* Add Event Dialog */}
      {showDialog && selectedProjectForDialog && (
        <AddEventDialog
          projectId={selectedProjectForDialog}
