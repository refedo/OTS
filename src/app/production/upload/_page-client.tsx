...
export default function UploadPartsPage() {
  ...
  const autoMap = (columns: string[]): Record<string, string> => {
    ...
    columns.forEach(col => {
      ...
      else if (lc === 'quantity' || lc === 'qty') mapping[col] = 'quantity';
      ...
      else if (lc.includes('area') && lc.includes('unit')) mapping[col] = 'netAreaPerUnit';
      else if (lc.includes('area') && lc.includes('total')) mapping[col] = 'netAreaTotal';
      else if (lc.includes('single') && lc.includes('weight')) mapping[col] = 'singlePartWeight';
      else if (lc.includes('weight') && lc.includes('total')) mapping[col] = 'netWeightTotal';
    });
    return mapping;
  };
