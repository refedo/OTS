...
export async function GET(req: Request) {
  ...
  try {
    const [assetRows, fixedAssetRows] = await Promise.all([
      ...
      prisma.$queryRawUnsafe<any[]>(`
        SELECT
          coa.account_code,
          coa.account_name,
          COALESCE(coa.account_category, 'Fixed Assets') as account_category,
          COALESCE(SUM(je.debit), 0) as total_debit,
          COALESCE(SUM(je.credit), 0) as total_credit,
          COALESCE(SUM(je.debit) - SUM(je.credit), 0) as balance
        FROM fin_journal_entries je
    ...
    return NextResponse.json({
      assets: assetRows.map(mapRow),
      fixedAssets: fixedAssetRows.map(mapRow),
    });
  } catch (error: any) {
