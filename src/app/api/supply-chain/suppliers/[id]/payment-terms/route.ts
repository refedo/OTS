...
import { z } from 'zod';
import {
  getSupplierPaymentTermsHistory,
  createSupplierPaymentTerms,
...
export const GET = withApiContext(async (req, session, ctx) => {
  ...
  return NextResponse.json(await getSupplierPaymentTermsHistory(id));
});
...
export const POST = withApiContext(async (req, session, ctx) => {
  ...
  const term = await createSupplierPaymentTerms(id, {
    ...parsed.data,
