...
import { z } from 'zod';
import {
  getSupplierCreditLimitHistory,
  createSupplierCreditLimit,
...
export const GET = withApiContext(async (req, session, ctx) => {
  ...
  return NextResponse.json(await getSupplierCreditLimitHistory(id));
});
...
export const POST = withApiContext(async (req, session, ctx) => {
  ...
  const row = await createSupplierCreditLimit(id, {
    ...parsed.data,
